...
const StyledInputCheck: React.FC<InputCheckProps> = styled(Box)<InputCheckProps>`
	...
	${props =>
		...
		css`
			&::before {
				border-color: ${getColorFromTheme(props.theme, props.theme.components.inputCheck.colors.default)};
				border-radius: ${props.theme.components.inputCheck.borderRadius.checkbox};
			}
			&::after {
				background-color: ${getColorFromTheme(props.theme, props.theme.components.inputCheck.colors.selected)};
				border: 0;
				height: 16px;
				left: 12px;
				top: calc(50% - 9px);
	...
	${props =>
		...
		css`
			&::before {
				border-color: ${getColorFromTheme(props.theme, props.theme.components.inputCheck.colors.selected)};
			}
			&::after {
				opacity: 1;
				transform: scale(1);
