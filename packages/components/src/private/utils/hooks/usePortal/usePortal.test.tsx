...
const renderTestComponent = (props: Partial<TestProps> = {}) => render(<TestComponent id={wrapperId} {...props} />);
...
describe('usePortal test', () => {
	...
	it('should create div inside wrapper div if div with passed id exists', () => {
		...
		expect(container.querySelector(`#${wrapperId} > .${testClass}`)).toBeInTheDocument();
	});

	it('should create div inside body div if div with passed id not exists', () => {
		const { baseElement } = renderTestComponent({ id: 'wrong' });
		expect(baseElement.querySelector(`body > .${testClass}`)).toBeInTheDocument();
	});
});
