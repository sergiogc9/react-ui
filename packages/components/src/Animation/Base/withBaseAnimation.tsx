...
const withBaseAnimation = <P extends {} = {}>(
  Component: React.ComponentType<P>,
  customAnimation?: Keyframes
) => {
  const StyledAnimationComponent: React.FC<BaseAnimationProps> = styled(
    Component
  )<BaseAnimationProps>`
  ...
  const AnimationComponent: React.FC<BaseAnimationProps<P>> = React.forwardRef<
    any,
    BaseAnimationProps<any>
  >(
    (
      {
        fillMode = 'both',
      ...
      const onAnimationEnded: BaseAnimationProps['onAnimationEnd'] = (
        event
      ) => {
        const element = innerRef.current;
