...
const Svg = forwardRef<SVGSVGElement, SvgProps>(
  ({ children, height, src, title, viewBox, width, ...props }, ref) => {
    ...
    const finalViewBox =
      ...
      (srcProps &&
        srcProps.width &&
        srcProps.height &&
        `0 0 ${srcProps.width} ${srcProps.height}`) ||
      (srcProps && srcProps.viewBox) ||
      '0 0 0 0';

    const [finalWidth, finalHeight] = [
      width ||
        (srcProps && srcProps.width) ||
        `${finalViewBox.split(' ')[2]}px`,
      height ||
        (srcProps && srcProps.height) ||
        `${finalViewBox.split(' ')[3]}px`
    ];
