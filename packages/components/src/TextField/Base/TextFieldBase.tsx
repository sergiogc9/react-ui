...
>(
  (
    ...
    const [currentValue, setCurrentValue] = useState(
      defaultValue?.toString().substr(0, maxLength) || ''
    );
      ...
      <StyledTextFieldBaseWrapper isDisabled={isDisabled} {...rest}>
        <StyledTextFieldBase labelPosition={labelPosition}>
          <Box position="absolute" height="100%" width="100%">
            <StyledTextFieldIconContent ml={3}>
              {leftContent}
            </StyledTextFieldIconContent>
            <TextFieldLabel
        ...
        >
          ...
          {!!maxLength && (
            <InputCounter
              isDisabled={isDisabled}
              maxLength={maxLength}
              valueContent={finalValue.toString()}
            />
          )}
        </Animation.BaseAnimation>
      </StyledTextFieldBaseWrapper>
