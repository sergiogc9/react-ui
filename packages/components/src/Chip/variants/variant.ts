...
const hoverHandler = (
  props: Partial<ChipGroupProps>,
  theme: DefaultTheme,
  variantProp: ChipGroupProps['variant']
) => {
  const hoverState = props.href ? 'hover' : 'default';

  if (props.bg)
    return { bg: props.bg, borderColor: props.borderColor || props.bg };
  return {
    bg: getColorFromTheme(
      theme,
      theme.components.chip.colors[variantProp!].background[hoverState]!
    ),
...
export default (props: StyledProps<ChipGroupProps>) => {
  const generateCSS = (variantProp: ChipGroupProps['variant']) => ({
    bg:
      props.bg ??
      props.theme.components.chip.colors[variantProp!].background.default,
    borderColor:
