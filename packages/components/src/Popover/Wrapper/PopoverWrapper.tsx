...
const translateSkidding = (
  skidding: PopoverWrapperProps['skidding'],
  placement: PopoverWrapperProps['placement']
) => {
  switch (placement) {
    case 'top-end':
    case 'bottom-end':
...
const PopoverWrapper: React.FC<PopoverWrapperProps> = ({
  ...
  const popoverPlacement = popoverContentRef.current
    ? (popoverContentRef.current.getAttribute(
        'data-placement'
      ) as PopoverWrapperProps['placement'])
    : undefined;
  ...
  return (
    <Tippy
      ...
      offset={[
        translateSkidding(skidding, popoverPlacement || placement),
        distance
      ]}
