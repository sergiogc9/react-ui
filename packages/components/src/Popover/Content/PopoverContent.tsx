...
const PopoverContent: React.FC<PopoverContentProps> = ({
  ...
  const onCustomMouseDown = React.useCallback<
    NonNullable<PopoverContentProps['onMouseDown']>
  >(
    (event) => {
  ...
  const onCustomTouchStart = React.useCallback<
    NonNullable<PopoverContentProps['onTouchStart']>
  >(
    (event) => {
      event.stopPropagation();
