...
const StepperStep: React.FC<StepperStepProps> = ({
  ...
  const { current, onChangeStep, showCheckIcon, steps, totalSteps, variant } =
  ...
  const onStepClicked = React.useCallback(() => {
    if (isAccessible && current !== contextValues.index && onChangeStep)
      onChangeStep(contextValues.index);
