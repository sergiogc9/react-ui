...
describe('StepperCircle component', () => {
	...
	it('should render small check icon for compacted variant', () => {
		renderStepperCircle({}, { current: 2, index: 1, showCheckIcon: true, variant: 'compacted' });
		expect(screen.getByTestId('stepper-circle-check-icon')).toHaveStyle(`width: ${theme.components.icon.sizes.s}px`);
	});
});
