...
const getButtonColorWithOpacity = (
  props: StyledProps<ButtonProps>,
  themeKey?: TLengthStyledSystem
) => {
  if (!themeKey) return undefined;

  const opacityPercent = props.isLoading ? 40 : 100;

  if (['link', 'subtle'].includes(props.variant!))
    return getColorFromTheme(props.theme, themeKey);
...
export default (props: StyledProps<ButtonProps>) => {
  const generateCSS = (variantProp: ButtonProps['variant']) => ({
    bg:
      props.bg ??
      getButtonColorWithOpacity(
        props,
        props.theme.components.button.colors[variantProp!].background.default
      ),
    color:
      props.color ??
      (props.theme.components.button.colors[variantProp!].text || 'neutral.0'),
    borderColor:
      props.borderColor ??
      (getButtonColorWithOpacity(
        props,
        props.theme.components.button.colors[variantProp!].border || ''
      ) ||
        'transparent'),
    borderRadius:
      props.borderRadius ??
      props.theme.components.button.borderRadius[variantProp!],
    fontWeight: 'bold',
    ...
    '&:focus-visible': {
      boxShadow: `0 0 0 3px ${
        ...
        getColorFromTheme(
          props.theme,
          props.theme.components.button.colors[variantProp!].focusShadow
        )
      }`,
      ...
      !props.isLoading && {
        '&:hover': {
          bg:
            ...
            getButtonColorWithOpacity(
              props,
              props.theme.components.button.colors[variantProp!].background
                .hover
            ),
          borderColor: 'transparent'
        }
    ...
    '&:active': !props.isDisabled &&
      !props.isLoading && {
        bg:
          ...
          getButtonColorWithOpacity(
            props,
            props.theme.components.button.colors[variantProp!].background.active
          ),
        borderColor: 'transparent'
      }
  });
