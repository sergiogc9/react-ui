...
const SelectField: React.FC<SelectFieldProps> = React.forwardRef<
  HTMLInputElement,
  SelectFieldProps
>(
  (
    ...
    const onTextFieldKeyPressed = React.useCallback<
      NonNullable<SelectFieldProps['onKeyDown']>
    >(
      (ev) => {
      ...
      return (
        <>
          ...
          <Icon
            ...
            data-testid={
              isOpen
                ? 'select-field-arrow-up-icon'
                : 'select-field-arrow-down-icon'
            }
            fill="neutral.400"
