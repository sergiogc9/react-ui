...
import {
  getPopoverContentMock,
  withTheme
} from 'components/private/utils/tests';
...
const renderSelect = (props: Partial<SelectProps> = {}) =>
  render(getSelectComponent(props));
...
describe('Select', () => {
  ...
  it('should show not results found if no options are provided', () => {
    ...
    expect(
      screen.getByText(theme.components.select.locales.en.no_results!)
    ).toBeInTheDocument();
  });
  ...
  it('should set default option', () => {
    ...
    expect(screen.getByText(optionTwo)).toHaveAttribute(
      'aria-selected',
      'true'
    );
  });
  ...
  it('should change option from controlled value', () => {
    ...
    expect(screen.getByText(optionThree)).toHaveAttribute(
      'aria-selected',
      'true'
    );
  ...
  it('should set two default options with multi select', () => {
    ...
    expect(screen.getByText(optionOne)).toHaveAttribute(
      'aria-selected',
      'true'
    );
    expect(screen.getByText(optionTwo)).toHaveAttribute(
  ...
  it('should change option from controlled value with multi select', () => {
    ...
    rerender(
      getSelectComponent({ isMultiSelect: true, value: [optionThreeID] })
    );
  ...
  it('should mark as bold matched text in filtered options when autocomplete', () => {
    ...
    expect(
      screen.getByTestId(optionTwoID).querySelector('strong')?.innerHTML
    ).toBe('celo');
  });
