...
const Select: React.FC<SelectProps> = ({
  ...
  const [inputValue, setInputValue] = React.useState(() =>
    getInputLabelFromSelectedOptions(selectedOptions)
  );
  ...
  useUpdateEffect(() => {
    const newSelectedOptions = value
      ? getSelectOptions(undefined, value, children)
      : {};
  ...
  useUpdateEffect(() => {
    if (!isAutocomplete || document.activeElement !== inputRef.current) {
      ...
      const newInputValue =
        getInputLabelFromSelectedOptions(newSelectedOptions);
  ...
  const onOptionClicked = React.useCallback<
    NonNullable<SelectOptionProps['onClick']>
  >(
    (ev) => {
      if (!isMultiSelect) {
        setIsOpen(false);
      ...
      setSelectedOptions((currentOptions) => {
        ...
        if (!isAutocomplete || !isMultiSelect)
          setInputValue(getInputLabelFromSelectedOptions(newOptions));
  ...
  const onAlphanumericKeyPressed = useFocusOptionWhilePressingKey(
    listBoxRef,
    isAutocomplete,
    setIsOpen
  );
  ...
  const onTextFieldChanged = React.useCallback<
    NonNullable<SelectFieldProps['onChange']>
  >(
    (ev) => {
  ...
  const onTextFieldRemoveBtnClicked = React.useCallback<
    NonNullable<SelectFieldProps['onRemoveButtonClick']>
  >(() => setSelectedOptions({}), []);
