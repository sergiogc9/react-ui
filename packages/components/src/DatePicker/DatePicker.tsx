...
const DatePicker: React.FC<DatePickerProps> = ({
  ...
  const [selectedDateRange, setSelectedDateRange] =
    React.useState(defaultDateRange);

  const onDayClicked = React.useCallback<
    NonNullable<DatePickerDayProps['onDayClick']>
  >(
    (day) => {
      if (isRange) {
  ...
  const onMonthClicked = React.useCallback<
    NonNullable<DatePickerMonthProps['onMonthClick']>
  >(
    (month) => {
      if (isRange) {
        let newRange: DateRange;
