...
const DatePickerMonth: React.FC<DatePickerMonthProps> = ({
  ...
  const [visibleYear, setVisibleYear] = React.useState(
    startOfYear(defaultVisibleYear ?? new Date())
  );
    ...
    return (
      <Box alignItems="center" justifyContent="space-between" marginBottom={1}>
        <IconButton
          ...
          isDisabled={
            minDate &&
            new Date(minDate).getFullYear() >= visibleYear.getFullYear()
          }
        ...
        <IconButton
          ...
          isDisabled={
            maxDate &&
            new Date(maxDate).getFullYear() <= visibleYear.getFullYear()
          }
          onClick={() => setVisibleYear((year) => changeYear(year, 1))}
        >
      ...
      <Box justifyContent="center" flexWrap="wrap">
        {monthsTexts.map((month, index) => {
          ...
          const isRangeStart =
            !!isRange &&
            !!dateRange &&
            isSameDay(startOfMonth(monthDate), startOfMonth(dateRange.start));
          const isRangeEnd =
            !!isRange &&
            !!dateRange &&
            isSameDay(startOfMonth(monthDate), startOfMonth(dateRange.end));
  ...
  return (
    <StyledDatePickerMonth
      width={theme.components.datePicker.day.size * 7}
      {...rest}
    >
      {yearContent}
      {monthsContent}
