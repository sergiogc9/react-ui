...
⚠️ IMPORTANT: The packages containing the icons **have to be installed by the consumer** and are not provided by the library. This way the consumer can decide what icon family to use, etc.

ℹ️ Remember that if you have a pro subscription, remember to configure the access to pro packages: [See oficial docs](https://fontawesome.com/v6/docs/web/setup/packages#configure-access)
