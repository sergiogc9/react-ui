...
<Story name="playground" parameters={getPlaygroundStoryParams({ include: ['columnGap', 'rowGap'] })}>
	{args => (
		<Grid {...args} width={800}>
			...
			<Grid.Box alignBoxs="center" bg="green.700" columns={4} height={150} initialColumn={9}>
				Grid.Box
			</Grid.Box>
...
<Canvas>
	<Story name="with custom box initial column" parameters={getFixedStoryParams()}>
		{args => (
			<Grid width={800}>
